...
export class CartPage extends BasePage {
  ...
  /** `Promotions.spec.ts` — valid “Sale” promo through thank-you + Continue Shopping. */
  async runPromotionsValidPromoCheckoutAfterCartOpen(): Promise<void> {
    ...
    await promoInput.press('CapsLock');
    await promoInput.fill('Sale');
    ...
    await cartPage.getByRole('listitem').filter({ hasText: 'COD - Cash On Delivery Cash' }).getByRole('radio').check();
    await expect(cartPage.getByText('COD - Cash On Delivery')).toBeVisible();
