...
/** Playwright tags for filtering (`--grep @smoke`, `--grep @commerce`). */
function cadSuiteTag(): string[] {
  const envTag = envConfig.envName === 'prod' ? '@prod' : '@qam';
  const cadTag = envConfig.envName === 'prod' ? '@cad-prod' : '@cad-qam';
  return [envTag, cadTag, '@commerce', '@order', '@tools', '@regression'];
}
...
test.describe(`CAD Site Version 1 — ${cadSuiteProfile()}`, { tag: cadSuiteTag() }, () => {
  ...
  test('negative flow: pub catalog — fake item URL shows not found or leaves catalog', async ({ page }) => {
    test.setTimeout(60_000);
